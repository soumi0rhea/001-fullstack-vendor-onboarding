  ...
  describe('addVendor', () => {
    ...
    describe('checkEmailExists', () => {
      it('returns result from API', async () => {
        ...
        const store = useVendorStore();
        const result = await store.checkEmailExists('test@company.com');

        expect(VendorService.checkEmailExists).toHaveBeenCalledWith('test@company.com');
      ...
      it('handles API errors correctly', async () => {
        ...
        vi.mocked(VendorService.checkEmailExists).mockRejectedValue(error);

        const store = useVendorStore();

        try {
          await store.checkEmailExists('test@company.com');
          expect(true).toBe(false);
        } catch (e) {
          expect(store.loading).toBe(false);
          expect(store.error).toBe('Failed to check email. Please try again later.');
        }
      });
    });
  });
